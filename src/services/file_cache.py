...
class FileCache:
    ...
    def _get_request_fingerprint(self) -> Optional[Dict[str, Any]]:
        """Đọc browser fingerprint gắn với request hiện tại."""
    ...
    async def _resolve_download_proxy(
        ...
        """Xác định proxy tải xuống theo loại media."""
        ...
        try:
            # Tải media (ảnh/video) ưu tiên proxy upload/download riêng
            ...
            # Các loại khác dùng proxy request
    ...
    def _guess_extension(self, url: str, media_type: str) -> str:
        """Giữ nguyên đuôi file gốc nếu có, không biết thì dùng giá trị mặc định."""
    ...
    def _build_download_headers(
        ...
        """Dựng header request tải media, ưu tiên tái dùng fingerprint trình duyệt đang giải Captcha."""
        headers = {
            "Accept": (
    ...
    def _write_cached_content(self, file_path: Path, content: bytes):
        """Ghi file tạm trước rồi thay thế atomically, tránh đọc file dở khi concurrency."""
    ...
    def _normalize_cache_error(self, error: Exception) -> str:
        """Chuẩn hóa lỗi cache, tránh bubble lỗi cấp thấp ra ngoài cho user."""
        if isinstance(error, FileNotFoundError):
            missing_name = Path(getattr(error, "filename", "") or "curl").name or "curl"
            return f"Máy chưa cài {missing_name}"
        ...
        if message.startswith("Failed to cache file:"):
            message = message.split(":", 1)[1].strip() or "Lỗi không xác định"
